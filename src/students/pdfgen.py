...
def creapdf(data: DataFrame, output_dir: str = "pdf") -> None:
    ...
    leader_name: str = str(data["Q02_Leader_name"][0]).strip()
    title: str = leader_name.replace(" ", "_") + ".pdf"
    file_path: str = join(output_dir, title)
    ...
    contenuto.append(Paragraph(
        f"Leader: {leader_name}", title_style
        ))
